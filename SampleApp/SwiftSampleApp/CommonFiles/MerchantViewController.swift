...
extension MerchantViewController: PayUCheckoutProDelegate {
    ...
    func generateHash(for param: DictOfString, onCompletion: @escaping PayUHashGenerationCompletion) {
        let hashString = (param[HashConstant.hashString] ?? "") + (saltTextField.text ?? "")
        let hash = Utils.sha512Hex(string: hashString)
        onCompletion([param[HashConstant.hashName] ?? "" : hash])
    }
